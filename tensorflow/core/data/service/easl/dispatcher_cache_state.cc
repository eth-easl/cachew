...
void CacheState::PrintContents() {
  VLOG(0) << "Printing the source_cache contents";
  for (auto x : is_source_cached_) {
    VLOG(0) << " > " << x.first << ": " << x.second;
  }

  VLOG(0) << "Printing the full_cache contents";
  for (auto x : is_cached_) {
    VLOG(0) << " > " << x.first << ": " << x.second;
  }
}
