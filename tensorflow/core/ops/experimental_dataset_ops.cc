...
REGISTER_OP("DataServiceDataset")
    ...
    .Input("scaling_threshold_up: float32") // EASL
    .Input("optimize_cost: bool") // EASL
    .Input("client_cost: float32") // EASL
    .Input("worker_cost: float32") // EASL
...
REGISTER_OP("ServiceCachePutDataset")
    .Input("input_dataset: variant")
    .Input("path: string")
    .Input("cache_format: int32")
    .Input("cache_compression: int32")
    .Input("parallelism: int32")
    .Output("handle: variant")
    .Attr("output_types: list(type) >= 1")
    .Attr("output_shapes: list(shape) >= 1")
    .SetShapeFn([](shape_inference::InferenceContext* c) {
